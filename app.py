    ...
    if user_doc:
        ...
        if user_doc.get("hevy_api_key"):
            ...
            if st.button("Update Hevy API Key"):
                ...
                if st.button("Save"):
                    # Store the key in session state for the form submission
                    st.session_state.new_hevy_key = new_key
                    st.rerun()
        else:
        ...
        with st.form("edit_profile"):
            ...
            if st.form_submit_button("Update Profile"):
                try:
                    ...
                    # Only update Hevy API key if a new one was provided
                    if (
                        "new_hevy_key" in st.session_state
                        and st.session_state.new_hevy_key
                    ):
                        encrypted_key = encrypt_api_key(st.session_state.new_hevy_key)
                        user_doc["hevy_api_key"] = encrypted_key
                        user_doc["hevy_api_key_updated_at"] = datetime.now(
                            timezone.utc
                        ).isoformat()
                        # Clear the session state after using it
                        st.session_state.new_hevy_key = None
        ...
        with st.form("edit_preferences"):
            ...
            if st.form_submit_button("Update Preferences"):
                try:
                    ...
                    # Save to database
                    doc_id, doc_rev = db.save_document(user_doc)
                    logging.info(f"Updated user preferences with ID: {doc_id}")
